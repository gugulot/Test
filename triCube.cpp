...
bool setupGraphics(int width, int height)
{
    ...
    text0 = new Text(resourceDirectory.c_str(), width, height);
    text1 = new Text(resourceDirectory.c_str(), width, height);
    text2 = new Text(resourceDirectory.c_str(), width, height);
    text3 = new Text(resourceDirectory.c_str(), width, height);
    text4 = new Text(resourceDirectory.c_str(), width, height);
    text5 = new Text(resourceDirectory.c_str(), width, height);
    text0->addString(800,800, "Zynq UltraScale+ MPSoC VCU Multimedia Demonstration", 255, 255, 255, 255);
    text1->addString(800,780, "Transcoding (H.264 -> H.265)", 255, 255, 255, 255);
    text2->addString(800,760, "Input Format: H.264/AVC", 255, 255, 255, 255);
    text3->addString(800,740, "Resolution  : UHD 4K (3840X2160)", 255, 255, 255, 255);
    text4->addString(800,720, "Frame Rate  : 30 FPS", 255, 255, 255, 255);
    text5->addString(800,700, "Video Format: Progressive, 4:2:0, 8 bpp", 255, 255, 255, 255);
...
int main(int argc, char **argv)
{
	...
	XStoreName (dpy, window, "Xilinx Inc - Test Application 4K");
	XMapWindow (dpy, window);

	int s = DefaultScreen(dpy);
	int y_offset = 20;
        const char* s2 = "(C)2012 Geeks3D.com"; 
        XDrawString(dpy, window, DefaultGC(dpy, s), 10, y_offset, s2, strlen(s2));
...
//	setupGraphics(WINDOW_WIDTH, WINDOW_HEIGHT);
	...
	while(!finish)
	{
		...
		if(!once){
		   setupGraphics(WINDOW_WIDTH, WINDOW_HEIGHT);	
		//   setupGraphics1(WINDOW_WIDTH, WINDOW_HEIGHT);	
		 //  setupGraphics2(WINDOW_WIDTH, WINDOW_HEIGHT);	
		 //  setupGraphics3(WINDOW_WIDTH, WINDOW_HEIGHT);	
		 //  setupGraphics4(WINDOW_WIDTH, WINDOW_HEIGHT);	
		 //  setupGraphics5(WINDOW_WIDTH, WINDOW_HEIGHT);	
		   once = true;
		}
		runGraphics(i);
